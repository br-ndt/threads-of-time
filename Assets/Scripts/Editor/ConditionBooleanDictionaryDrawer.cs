...
/// <summary>
/// Custom PropertyDrawer for the ConditionBooleanDictionary class.
/// By targeting the concrete class instead of the generic one, we ensure this drawer is always used.
/// </summary>
[CustomPropertyDrawer(typeof(ConditionBooleanDictionary))]
public class ConditionBooleanDictionaryDrawer : PropertyDrawer
{
    ...
    private void AddNewEntry(SerializedProperty keys, SerializedProperty values, int[] allEnumValues)
    {
        ...
        if (firstAvailableKey != -1)
        {
            ...
            values.GetArrayElementAtIndex(values.arraySize - 1).boolValue = false; // Default value
        }
    }
